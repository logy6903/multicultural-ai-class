...
type Step = "code" | "group" | "name" | "room";
type LeftTab = "activity" | "reflection";
type RightTab = "overview" | "chat" | "roles";
...
export default function GroupPage() {
  ...
  const [lessonGroups, setLessonGroups] = useState<Group[]>([]);
  ...
  async function lookupLesson(c: string) {
    try {
      setError("");
      const data = await api("listLessonGroups", { code: c });
      setLesson(data.lesson);
      setLessonGroups(data.groups);
      setStep("group");
    } catch (e) {
  ...
  async function joinAndEnter() {
    if (!selectedGroupId || !studentName.trim()) {
      setError("이름을 입력하세요.");
      return;
    }
    try {
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      // Realtime 으로 메시지 도착 → setMessages 자동 반영
    } catch (e) {
      setError(e instanceof Error ? e.message : "오류");
    } finally {
      setLoading(false);
    ...
    return (
      <EntryShell error={error}>
        ...
        <div className="mt-5 grid gap-2">
          {lessonGroups.map((g) => {
            const full = (g.memberCount ?? 0) >= g.capacity;
            return (
              <button
                key={g.id}
                onClick={() => !full && selectGroup(g.id)}
                disabled={full}
              ...
              >
                ...
                <span className="text-xs">
                  {g.memberCount ?? 0}/{g.capacity}명{full ? " · 가득참" : ""}
                </span>
              </button>
            );
          })}
  ...
  if (step === "name" && lesson) {
    const sg = lessonGroups.find((g) => g.id === selectedGroupId);
    return (
      <EntryShell error={error}>
        <p className="text-xs font-semibold text-indigo-600">
          {lesson.title} · {sg?.name}
        </p>
        <h1 className="mt-1 text-2xl font-bold text-slate-900">이름 입력</h1>
        <p className="mt-2 text-sm text-slate-600">
          모둠에서 표시될 자기 이름을 입력하세요.
        </p>
        ...
        <label className="mt-5 block">
          ...
          <input
            ...
            placeholder="홍길동"
            onKeyDown={(e) => e.key === "Enter" && joinAndEnter()}
            autoFocus
          />
        </label>

        <button
          onClick={joinAndEnter}
  ...
  return (
    <main className="flex h-screen flex-col overflow-hidden bg-slate-50">
      ...
      {/* 본문 16:9 */}
      <div className="grid min-h-0 flex-1 grid-cols-[1fr_1.3fr] gap-3 p-3">
        {/* 좌: 개인 입력 (활동기록 / 성찰문) */}
        <section className="flex min-h-0 flex-col overflow-hidden rounded-2xl bg-white shadow-sm">
          <div className="flex shrink-0 border-b border-slate-200">
            ...
            <TabButton
              active={leftTab === "reflection"}
              onClick={() => setLeftTab("reflection")}
            >
              성찰문
            </TabButton>
          </div>
